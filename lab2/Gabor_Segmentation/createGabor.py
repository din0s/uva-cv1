...
CREATEGABOR Creates a complex valued Gabor filter.
  ...
  - OUTPUT
    ...
    myGabor_real = np.multiply(gaussianEnv, cosCarrier)
    myGabor_imaginary = np.multiply(gaussianEnv, sinCarrier)
...
def generateRotationMatrix(theta):
    ...
    rotMat = np.array([[np.cos(theta), -1*np.sin(theta)], [np.sin(theta), np.cos(theta)]])
    return rotMat
...
def createCos(rot_x, lamda, psi):
    ...
    cosCarrier = np.cos((2 * np.pi / lamda) * rot_x + psi)
...
def createSin(rot_x, lamda, psi):
    ...
    sinCarrier = np.sin((2 * np.pi / lamda) * rot_x + psi)
