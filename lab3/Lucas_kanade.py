...
def lucas_kanade(image1: np.ndarray, image2: np.ndarray) -> None:
    ...
    blocks1 = blockify(image1, STRIDE)
    ...
    vs = [np.linalg.inv(A.T @ A) @ A.T @ b for (A, b) in zip(As, Bs)]
    plt.imshow(image1, cmap='gray')
    centers = [[(x, y) for y in range(7, image1.shape[0] - image1.shape[0] % 15, 15)] for x in range(7, image1.shape[1] - image1.shape[1] % 15, 15)]
    centers = sum(centers, [])
    x, y = np.array([l[0] for l in centers]), np.array([l[1] for l in centers])
    plt.imshow(image1, cmap='gray')
    plt.quiver(x, y, np.array(vs)[:, 0], np.array(vs)[:, 1], angles='xy', scale_units='xy', scale=50)
    plt.show()
